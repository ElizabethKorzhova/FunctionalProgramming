  ...
  return (
    <main className={`container ${styles.labContainer}`}>
      ...
      <ul>
        ...
        <li className={styles.contentItem}>
          <a className={styles.contentItemLink} href={""}>
            Умова задачі
          </a>
        </li>
        <li className={styles.contentItem}>
    ...
    (define (exponent-times acc previous current k) ; визначаємо допоміжну функцію для хвостової 
    рекурсії. acc - аккумулятор. previous - попередній елемент. current - поточний елемент. 
    k - індекс елементу
        (if (and (< (abs (- previous current)) e) (< current 1)) ; якщо різниця миж поточним 
        та попереднім елементом менша за епсілон, та поточний елемент менший за одиницю 
        (потрібно, тому що 2 підряд елементи ряду можуть бути однаковими, як наприклад у х = 2)
    ...
    (exact->inexact (exponent-times 0 0 1 1)) ; викликаємо допоміжну функцію з початковими значеннями, 
    та приводи дріб до числа з плаваючою крапкою
)
...
(define (function x e) ; визначаємо функцію згідно з варіантом (12)
    (if (and (<= -2 x) (<= x 0)) ; якщо -2 <= x <= 0
        ...
        (if (and (< 0 x) (< x 2)) ; інакше, якщо 0 < x < 2
            ...
            "Incorrect value of x") ; інакше, вивід в консоль, що х задан неправильно, тому що виходить 
            за дозволені межі
    )
)
...
(define (first-digit n) ; визначаємо функцію для пошуку першої цифри в числі
    (if (< n 10) ; якщо число меньше за 10
        ...
        (first-digit (quotient n 10)) ; інакше, рекурсивно ділимо націло на 10, 
        допоки не отримаємо число менше 10
    )
)

(define (sum-of-digits n) ; визначаємо функцію для пошуку суми цифр в числі
    (define (sum-of-digits-times acc n) ; визначаємо допоміжну функцію для хвостової рекурсії. 
    асс - акумулятор, тобто сума цифр. n - поточне число
        (if (< n 10) ; якщо числе менше за 10
            (+ acc n) ; то це була перша цифра, плюсуємо до попередньої суми та повертаємо
            (sum-of-digits-times
                                 (+ acc (modulo n 10)) ; новий акумулятор - сума старого 
                                 акумулятора та останньої цифри в числі
...
`}
      ...
      <div className={styles.conclusion}>
        В результаті роботи було вирішено два задання за допомогою мови Schema.
